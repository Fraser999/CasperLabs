...
use types::{encoding, gens, URef, KEY_LOCAL_SEED_LENGTH};
...
mod basics {
    ...
    proptest! {
        ...
        fn key_should_roundtrip(key in test_key_arb()) {
            encoding::test_serialization_roundtrip(&key)
        }
    }
}
...
) -> Result<(Blake2bHash, Vec<HashedTrie<TestKey, TestValue>>), encoding::Error> {
